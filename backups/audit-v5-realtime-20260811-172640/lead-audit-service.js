...
export function createLeadAuditService({ store, workspaceService, reportTemplateProvider } = {}) {
  ...
  function queueMiniBatch(user, input = {}) {
    // Production default-audit prefetch. This accepts a full caller day (normally
    // 100 leads) but persists every queued audit in ONE store transaction.
    // Workers still run at a hard global concurrency of two.
  ...
  function resumeQueuedReports() {
    ...
    store.update((draft) => {
      ...
      for (const item of draft.leadAudits) {
        ...
        // After a process restart there is no live worker for an old generating
        // record, so it is safe to put it back in the queue.
    ...
    for (const id of ids) enqueue(id, { front: false });
  }
  ...
  function recoverStaleReports() {
    ...
    for (const id of [...staleIds, ...queued]) {
      enqueue(id, { front: false });
    }

    if (staleIds.length) {
      auditLog("stale-recovered", {
