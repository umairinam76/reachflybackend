...
import { createCodesyncPlatformAdminService } from "./codesync-platform-admin-service.js";
...
// Supabase/Postgres must be loaded before any service, seed, queue, or socket
// reads application state. All request-time reads remain in-process and fast.
await store.ready();
...
const voiceCommerceService =
  createVoiceCommerceService({
    store,
    workspaceService,
    emit({ workspaceId, event, payload }) {
      emitToWorkspace(workspaceId, event, payload);
...
const creditBillingService =
  createCreditBillingService({
    store,
    workspaceService,
    externalSafepayHandler: (input) =>
      voiceCommerceService.handleVerifiedSafepayEvent(input),
  });

const workspaceConnectionsService =
  createWorkspaceConnectionsService({
    store,
    workspaceService,
    email,
  });
...
const codesyncPlatformAdminService =
  createCodesyncPlatformAdminService({
    store,
    creditBillingService,
  });
...
app.use(
  express.json({
    ...
    verify(req, _res, buffer) {
      // Provider webhook signatures must be verified against the exact raw body.
      if (
        req.originalUrl?.startsWith("/api/telnyx/") ||
        req.originalUrl?.startsWith("/api/webhooks/safepay")
      ) {
...
async function verifyGoogleIdentityCredential(credentialValue) {
  const credential = String(credentialValue || "").trim();
  const clientId = String(process.env.GOOGLE_AUTH_CLIENT_ID || "").trim();
  ...
  if (!credential) {
    const error = new Error("Google authentication credential is required.");
    error.statusCode = 400;
  ...
  if (!payload.sub || !email || payload.email_verified !== true) {
    ...
    error.code = "GOOGLE_IDENTITY_NOT_VERIFIED";
    throw error;
  }
...
function requireAuth(req, res, next) {
  ...
  try {
    ...
    if (user.blockedAt || user.blocked === true || user.status === "blocked") {
      return res.status(403).json({
  ...
  (req, res) => {
    try {
      ...
      const signupWorkspaceId = String(
        signupWorkspaceContext.workspaceId ||
          workspaceUser.workspaceId ||
          workspaceUser.companyId ||
          workspaceUser.id ||
          ""
      ).trim();
...
app.post(
  "/api/auth/google",
  asyncRoute(async (req, res) => {
    const identity = await verifyGoogleIdentityCredential(req.body?.credential);
    ...
    if (user) {
      if (user.blockedAt || user.blocked === true || user.status === "blocked") {
        const error = new Error("This account is currently blocked.");
        error.statusCode = 403;
        error.code = "ACCOUNT_BLOCKED";
        throw error;
      }
      if (user.googleSub && String(user.googleSub) !== identity.sub) {
        const error = new Error(
          "This email is already linked to a different Google identity. Sign in with your existing method and review account security."
        );
      ...
      store.update((draft) => {
        ...
        if (!target) return;
        target.googleSub = identity.sub;
        target.googleAuth = true;
        target.avatarUrl = target.avatarUrl || identity.picture;
      ...
      if (mode !== "signup") {
        const error = new Error(
          "No ReachFly account exists for this Google account. Create your workspace first."
        );
      ...
      if (accountType === "company" && !companyName) {
        const error = new Error("Company name is required for a company workspace.");
        error.statusCode = 400;
      ...
      user = {
        ...
        role:
          descriptiveRole ||
          (accountType === "company" ? "Owner" : "Founder"),
      ...
      store.update((draft) => {
        ...
        draft.activity.unshift({
          id: crypto.randomUUID(),
          type: "auth",
          title: `${user.name} created a workspace with Google`,
          createdAt: now,
        });
      });
      created = true;
    ...
    const workspaceUser = workspaceService.ensureWorkspaceForUser(user.id) || user;
    if (created) {
      const ctx = workspaceService.getContext?.(workspaceUser) || {};
    ...
    const token = signToken({ userId: workspaceUser.id });
    res.status(created ? 201 : 200).json({
      token,
      user: publicUser(workspaceUser),
      created,
      onboardingRequired: created,
    });
  })
);
...
/* ==========================================================
   Workspace connections — Google Workspace email/calendar
   ========================================================== */
...
app.post(
  ...
  (req, res) => {
    res.json(
      workspaceConnectionsService.beginGoogleOAuth(req.user, req.body || {})
    );
  }
);
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      const result = await workspaceConnectionsService.handleGoogleOAuthCallback({
        code: req.query?.code,
        state: req.query?.state,
      });
      returnTo = result?.returnTo || returnTo;
      const url = new URL(returnTo, APP_URL);
      url.searchParams.set("google_connection", "success");
      return res.redirect(302, url.toString());
    } catch (error) {
      const url = new URL(returnTo, APP_URL);
      url.searchParams.set("google_connection", "error");
      url.searchParams.set(
        "reason",
        String(error?.code || "connection_failed").slice(0, 100)
      );
      return res.redirect(302, url.toString());
    }
  }
...
app.delete(
  ...
  (req, res) => {
    res.json(
      workspaceConnectionsService.disconnect(req.user, req.params.connectionId)
    );
  }
);

app.post(
  "/api/connections/:connectionId/test-email",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      await workspaceConnectionsService.testEmail(
        req.user,
...
app.post(
  "/api/connections/:connectionId/test-calendar",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      await workspaceConnectionsService.testCalendar(
        req.user,
        req.params.connectionId,
        req.body || {}
      )
...
async function sendPasswordResetEmail({ to, resetUrl }) {
  ...
  const fromEmail = String(
    process.env.AUTH_SMTP_FROM_EMAIL || process.env.AUTH_SMTP_USERNAME || ""
  ).trim();
  ...
  // Reuse the platform owner's linked ReachFly Emailbox when dedicated auth
  // SMTP is not configured. Passwords remain server-side in the Emailbox service.
  const platformOwner = (store.read().users || []).find(
    (item) => normalizeEmail(item.email) === "owner@codesynclabs.com"
  );
  if (platformOwner?.id && email?.sendCampaignEmail) {
    const settings = email.getSettings?.(platformOwner.id) || {};
    const accounts = Array.isArray(settings.accounts) ? settings.accounts : [];
    const accountId = settings.activeAccountId || accounts[0]?.id || "";
    if (accountId) {
      await email.sendCampaignEmail(platformOwner.id, {
        accountId,
        to,
        subject,
        body: text,
  ...
  const error = new Error(
    "Password recovery email is not configured. Configure the platform Emailbox or AUTH_SMTP_* settings."
  );
  error.statusCode = 503;
  error.code = "PASSWORD_EMAIL_NOT_CONFIGURED";
  throw error;
}
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    const user = (state.users || []).find(
      (item) => normalizeEmail(item.email) === emailValue
    );
    if (
      !user ||
      ...
      user.blockedAt ||
      user.blocked === true ||
      String(user.status || "").trim().toLowerCase() === "blocked"
    ) {
    ...
    store.update((draft) => {
      draft.passwordResets = Array.isArray(draft.passwordResets)
        ? draft.passwordResets
        : [];
      draft.passwordResets = draft.passwordResets.filter(
        (item) =>
          item.userId !== user.id ||
          item.usedAt ||
    ...
    await sendPasswordResetEmail({ to: emailValue, resetUrl });
    return res.json({ message });
  })
);
  ...
  async (req, res) => {
    ...
    try {
      leadCreditReservation = creditBillingService.reserveUsage({
        ...
        quantity: limit,
        idempotencyKey: `lead-stream:${req.requestId || crypto.randomUUID()}`,
        actorId: req.user?.id || "lead-search",
        metadata: { niche, location, requested: limit, mode: "stream" },
      });
    } catch (billingError) {
      return res.status(billingError?.statusCode || 402).json({
        error: billingError?.message || "Not enough ReachFly credits.",
        code: billingError?.code || "INSUFFICIENT_CREDITS",
        details: billingError?.details || null,
      });
    }
    ...
    try {
      ...
      if (leadCreditReservation?.id) {
        creditBillingService.commitUsage(leadCreditReservation.id, {
          actualQuantity: Number(result?.delivered || result?.leads?.length || 0),
          metadata: { status: result?.status || "complete" },
        });
      }
      ...
      if (
        ...
        if (leadCreditReservation?.id) {
          creditBillingService.commitUsage(leadCreditReservation.id, {
            ...
            metadata: { status: partialResult.status, timedOut: true },
          });
        }
  ...
  async (req, res) => {
    ...
    try {
      ...
      leadCreditReservation = creditBillingService.reserveUsage({
        ...
        feature: "lead_generated",
        quantity: limit,
        idempotencyKey: `lead-search:${req.requestId || crypto.randomUUID()}`,
        actorId: req.user?.id || "lead-search",
        metadata: { niche, location, requested: limit, mode: "json" },
      });
  ...
  async (req, res, next) => {
    ...
    try {
      creditReservation = creditBillingService.reserveUsage({
        ...
        quantity: 1,
        idempotencyKey: `ai-command:${req.requestId || crypto.randomUUID()}`,
...
/* ==========================================================
   ReachFly usage credits + Safepay top-ups
   ========================================================== */
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    res.json(creditBillingService.getBillingDashboard(req.user));
  })
);
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    res.status(201).json(
      await creditBillingService.createCreditCheckout(
        req.user,
        req.body || {}
      )
    );
...
app.post(
  "/api/billing/ai-calling/checkout",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.status(201).json(
      await creditBillingService.createAiCallCreditCheckout(
        req.user,
        req.body || {}
      )
    );
...
app.post(
  ...
  asyncRoute(async (req, res) => {
    const result = await creditBillingService.handleSafepayWebhook({
      rawBody: req.rawBody || JSON.stringify(req.body || {}),
      headers: req.headers,
      body: req.body || {},
    });
...
/* ==========================================================
   Paid Voice onboarding: number purchase and provisioning
   ========================================================== */
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    res.set("Cache-Control", "no-store");
    res.json(voiceCommerceService.getDashboard(req.user));
  })
);

app.post(
  "/api/voice-commerce/numbers/search",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.set("Cache-Control", "no-store");
    res.json(
      await voiceCommerceService.searchAvailableNumbers(
        req.user,
        req.body || {}
      )
    );
...
app.post(
  "/api/voice-commerce/numbers/checkout",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.status(201).json(
      await voiceCommerceService.createNumberCheckout(
        req.user,
        req.body || {}
      )
    );
...
app.post(
  "/api/voice-commerce/numbers/existing",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.status(201).json(
      await voiceCommerceService.connectExistingNumber(
        req.user,
        req.body || {}
      )
    );
...
app.post(
  "/api/voice-commerce/numbers/:id/verify",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      await voiceCommerceService.verifyExistingNumber(
        req.user,
        req.params.id,
        req.body || {}
      )
...
app.post(
  "/api/voice-commerce/numbers/:id/routing-test",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
...
app.post(
  "/api/voice-commerce/bundles/checkout",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.status(201).json(
      await voiceCommerceService.createBundleCheckout(
        req.user,
        req.body || {}
      )
    );
...
app.get(
  "/api/voice-commerce/orders/:id",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.set("Cache-Control", "no-store");
    res.json(
      voiceCommerceService.getOrder(
        req.user,
        req.params.id
      )
    );
...
app.post(
  "/api/voice-commerce/orders/:id/retry",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      await voiceCommerceService.retryProvision(
        req.user,
        req.params.id
      )
    );
...
app.get(
  "/api/platform-admin/access",
  requireAuth,
  asyncRoute(async (req, res) => {
    const access = codesyncPlatformAdminService.getAccess(req.user);
    if (!access.available) {
      return res.status(404).json({ error: access.reason || "Not found." });
    }
    res.json(access);
  })
);
...
app.get(
  ...
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(codesyncPlatformAdminService.getDashboard(req.user));
  })
);

app.patch(
  "/api/platform-admin/marketing-leads/:accountId",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      codesyncPlatformAdminService.updateMarketingLead(
        req.user,
...
app.put(
  "/api/platform-admin/credit-rates/:feature",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      codesyncPlatformAdminService.updateCreditRate(
        req.user,
        req.params.feature,
        req.body || {}
      )
...
app.put(
  "/api/platform-admin/credit-packs/:packId",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      codesyncPlatformAdminService.updateCreditPack(
        req.user,
        req.params.packId,
        req.body || {}
      )
...
app.post(
  "/api/platform-admin/credits/:workspaceId/adjust",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      codesyncPlatformAdminService.adjustCredits(
        req.user,
...
app.get(
  "/api/telnyx/ai-agent/agents",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      await telnyxAiAgentService.listAgents(req.user, {
        force: String(req.query.force || "") === "true",
      })
    );
  })
...
function isMissingAssignedCalendarConnectionError(error) {
  return [
...
/*
 * Voice-agent tool routes are provider-facing and never require a customer
 * bearer token. Each handler validates the configured agent-tool secret.
 */
/*
 * These tool routes are called by the managed voice-agent runtime itself.
 * They are protected by TELNYX_AI_AGENT_TOOL_SECRET configured as a
 * custom header on each Telnyx webhook tool.
 */
app.post(
  "/api/telnyx/ai-agent/tools/send-email",
  asyncRoute(async (req, res) => {
    res.json(
      await workspaceConnectionsService.sendAgentEmail({
        headers: req.headers,
...
app.post(
  "/api/telnyx/ai-agent/tools/google-calendar/check",
  asyncRoute(async (req, res) => {
    res.json(
      await workspaceConnectionsService.checkAgentCalendar({
        headers: req.headers,
        body: { ...(req.query || {}), ...(req.body || {}) },
      })
    );
...
app.post(
  "/api/telnyx/ai-agent/tools/check-calendar",
  asyncRoute(async (req, res) => {
    try {
      return res.json(
        await workspaceConnectionsService.checkAgentCalendar({
    ...
    res.json(
      await telnyxAiAgentService.checkCalendar({
        headers: req.headers,
        body: req.body || {},
      })
    );
...
app.post(
  "/api/telnyx/ai-agent/tools/critical-live-data",
  asyncRoute(async (req, res) => {
    res.json(
...
/*
 * ElevenLabs conversation-initiation webhook for inbound SIP calls.
 * ElevenLabs sends the workspace-stored secret in
 * X-ReachFly-Inbound-Secret. Do not add requireAuth here.
 */
app.post(
  "/api/telnyx/ai-agent/elevenlabs/inbound-init",
  asyncRoute(async (req, res) => {
    res.json(
      await telnyxAiAgentService.handleElevenLabsInboundInit({
        headers: req.headers,
        body: req.body || {},
      })
    );
...
/*
 * ElevenLabs signed post-call webhooks. express.json() already captures
 * req.rawBody for every /api/telnyx/* route so provider signature
 * verification uses the exact request bytes.
 */
app.post(
  "/api/telnyx/ai-agent/elevenlabs/webhooks",
  asyncRoute(async (req, res) => {
    res.status(200).json(
      await telnyxAiAgentService.handleElevenLabsWebhook({
        rawBody:
...
app.post(
  "/api/telnyx/ai-agent/calls/:id/sync",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      await telnyxAiAgentService.syncElevenLabsConversation(
        req.user,
        req.params.id
      )
    );
...
app.get(
  "/api/telnyx/ai-agent/calls/:id/monitoring",
  requireAuth,
  asyncRoute(async (req, res) => {
    res.json(
      telnyxAiAgentService.getCallMonitoringCapabilities(
        req.user,
        req.params.id
      )
    );
...
app.get(
  "/api/telnyx/ai-agent/calls/:id/audio",
  requireAuth,
  asyncRoute(async (req, res) => {
    const result =
      await telnyxAiAgentService.getElevenLabsCallAudio(
        req.user,
        req.params.id
      );
...
async function shutdownReachFly(signal) {
  console.log(
    `[shutdown] ${signal} received; stopping schedulers and flushing Supabase writes.`
  );
  ...
  try {
    ...
    console.error(
      `[shutdown] Supabase flush failed: ${error?.message || error}`
    );
  }
...
process.once("SIGINT", () => {
  void shutdownReachFly("SIGINT");
});

process.once("SIGTERM", () => {
  void shutdownReachFly("SIGTERM");
});
