...
// Website and GMB are audit TRACKS, not report types.
// Both tracks expose exactly these three reports.
const REPORT_KINDS = [
  "mini",
  "competitor",
  "full",
...
const DEFAULT_TEMPLATES = {
  ...
  mini: {
    name: "Mini Audit",
    enabled: true,
    lengthGuidance: "One screen / one page; under 15 seconds to skim before dialing",
    instructions:
      "DEFAULT for every Website and GMB lead. Keep Website and GMB findings separate. Website Mini: business + URL, current search standing, mobile PageSpeed when confirmed, strongest hook, up to 3 verified talking points, and a suggested opener. GMB Mini: business/category/phone, rating + review count when confirmed, local standing, strongest gap versus a verified nearby competitor, up to 3 verified talking points, and a suggested opener. Missing facts must say 'Not confirmed — verify on call'. Do not fabricate rankings, scores, review counts, competitors, hours, addresses, technologies, or metrics. No manager PDF is required.",
  },
  competitor: {
    name: "Competitor Analysis",
    enabled: true,
    lengthGuidance: "2-4 pages; client-facing, factual comparison with no recommendations or internal pitch",
    instructions:
      "Website track: exact customer search, target visibility, 3 competitors ranking above the target (or top 3), local/service page depth, directory/review-platform presence, conversion/trust signals, technical health best-effort, weighted score comparison, and a descriptive closing synthesis. GMB track: rating/reviews/hours/score at a glance, 5-7 verified nearby competitors when possible, profile findings with why-it-matters, and a descriptive closing synthesis. Keep tracks separate. No internal service pitch, pricing, recommendations, or invented competitor data.",
  },
  full: {
    name: "Full Audit",
    enabled: true,
    lengthGuidance: "4-8 pages; internal-only, detailed evidence-grounded opportunity analysis",
    instructions:
      "INTERNAL ONLY. Website track: business identification, weighted Website score, live PageSpeed mobile/desktop, SEO/accessibility/best-practices scores, Core Web Vitals, HTTPS, viewport, title/meta/H1, alt text, canonical, sitemap, robots, schema, page weight/request count when confirmed, SEO/trust findings, competitors, keywords/ranking tests, scorecard, not-yet-assessed items, and opportunity/build plan. GMB track: business identification, weighted GMB score, issues, review analysis, 5-7 nearby competitors when possible, keywords, local SEO/prominence, ranking opportunity, scorecard, not-yet-assessed items, and GMB opportunity/build plan. Recommendations are allowed only here. Never estimate technical metrics or fabricate public evidence.",
  },
};

const DEFAULT_MANAGER_SYSTEM_PROMPT =
  "Website and GMB are independent evidence tracks. Every lead uses Mini Audit by default, with Competitor Analysis and Full Audit available on demand. Write in a mature, professional, commercially useful tone. Missing facts must be marked 'Not confirmed — verify on call'. Competitor Analysis is client-facing and descriptive only. Full Audit is internal-only. Optional PDFs may control layout/style only and must never weaken ReachFly's factual-verification rules.";
    ...
    if (activeUsable) {
      return {
        ...
        name:
          active.name ||
          defaults.name,
        // Report availability is a product rule; manager settings customize presentation only.
        enabled: true,
        lengthGuidance:
          active.lengthGuidance ||
          defaults.lengthGuidance,
...
function normalizeKind(value) {
  ...
  if (!kind) {
    throw httpError(
      400,
      "Report type must be mini, competitor, or full (legacy website/gmb aliases are retained only for old template history)."
    );
  }
