...
/**
 * Structured voice-call diagnostics.
 *
 * PM2 usage:
 *   pm2 logs reachfly-api --lines 500 | grep "\\[voice-call\\]"
 *
 * Privacy:
 * - no API keys / tool secrets
 * - no prompt bodies / CRM notes
 * - phone numbers are masked to the final four digits
 */
...
function elapsedSeconds(fromValue, toValue = Date.now()) {
  const fromMs =
    typeof fromValue === "number"
      ? fromValue
      : Date.parse(String(fromValue || ""));
  const toMs =
    typeof toValue === "number"
      ? toValue
      : Date.parse(String(toValue || ""));
  ...
  return Math.max(
    0,
    Math.round(((toMs - fromMs) / 1000) * 100) / 100
  );
}
...
export function createTelnyxAIAgentService({
  ...
  function getAccess(user) {
    ...
    const codesyncWorkspace = isCodesyncLabs(ctx, user);
    const hidden = !codesyncWorkspace;
    ...
    return {
      ...
      role,
      accountType: "company",
      reason: !codesyncWorkspace
        ? "The AI caller is available only to the CodeSync Labs workspace."
        : !authorized
          ? "CodeSync Labs owner, administrator or manager access is required."
          : !globallyEnabled
            ? "The Telnyx voice-agent service is disabled by the server configuration."
            : featureSetting === false
              ? "The Telnyx voice agent is disabled for CodeSync Labs."
              : "",
    };
  }
  ...
  async function analyzeWebsite(user, input = {}) {
    ...
    voiceCallLog("agent.saved", {
      workspaceId: ctx.workspaceId,
      elevenLabsAgentId,
      phoneNumberId: elevenLabsPhoneNumberId,
      voiceId: saved?.voice || "",
      voiceGender: saved?.voiceGender || "",
      salesCallerName: saved?.salesCallerName || "",
      ttsModel: saved?.elevenLabsTtsModel || "",
      turnEagerness: saved?.elevenLabsTurnEagerness || "",
      turnTimeoutSeconds: saved?.elevenLabsTurnTimeoutSeconds || 0,
      silenceEndCallTimeout:
        saved?.elevenLabsSilenceEndCallTimeout ?? null,
      maxCallSeconds: saved?.maxCallSeconds || 0,
    });
  ...
  async function saveAgent(user, input = {}) {
    ...
    /*
     * Resolve the selected ElevenLabs voice before we build the live prompt.
     * Male voice -> James
     * Female voice -> Jessica
     *
     * Voice metadata is read from ElevenLabs labels when available.
     */
    const selectedVoiceId =
      clean(config.voice) ||
      clean(currentTts.voice_id) ||
      DEFAULT_VOICE;
    const salesVoiceIdentity = await resolveSalesVoiceIdentity({
      voiceId: selectedVoiceId,
      existing,
    });

    config.voice = selectedVoiceId;
    config.voiceGender = salesVoiceIdentity.gender;
    config.voiceProviderName = salesVoiceIdentity.voiceName;
    config.salesCallerName = salesVoiceIdentity.callerName;
    config.greeting = buildSalesHeadGreeting(config);

    voiceCallLog("agent.voice-resolved", {
      workspaceId: ctx.workspaceId,
      elevenLabsAgentId,
      voiceId: selectedVoiceId,
      voiceName: salesVoiceIdentity.voiceName,
      voiceGender: salesVoiceIdentity.gender,
      salesCallerName: salesVoiceIdentity.callerName,
    });
      ...
      `/v1/convai/agents/${encodeURIComponent(elevenLabsAgentId)}`,
      {
        ...
        body: {
          ...
          conversation_config: {
            ...
            /*
             * Natural consultative-sales turn taking.
             *
             * silence_end_call_timeout is explicitly disabled so a stale
             * ElevenLabs dashboard value cannot auto-end a quiet call.
             */
            turn: {
              turn_timeout: clampInteger(
              ...
              turn_eagerness:
                clean(process.env.ELEVENLABS_TURN_EAGERNESS) ||
                "normal",
              turn_model: "turn_v3",
            },
            tts: {
              voice_id:
                ...
                DEFAULT_VOICE,
              /*
               * Eleven v3 Conversational is the expressive conversational
               * default. Override with ELEVENLABS_TTS_MODEL_ID when needed.
               */
              model_id:
                clean(process.env.ELEVENLABS_TTS_MODEL_ID) ||
                "eleven_v3_conversational",
              optimize_streaming_latency: clampInteger(
    ...
    store.update((draft) => {
      ...
      Object.assign(agent, {
        ...
        telnyxVersionId: "",
        elevenLabsTtsModel:
          clean(syncedTts.model_id) ||
          clean(process.env.ELEVENLABS_TTS_MODEL_ID) ||
          "eleven_v3_conversational",
        elevenLabsTurnEagerness:
          clean(process.env.ELEVENLABS_TURN_EAGERNESS) ||
          "normal",
        elevenLabsTurnTimeoutSeconds: clampInteger(
          process.env.ELEVENLABS_TURN_TIMEOUT_SECONDS,
          8,
          5,
          30
        ...
        elevenLabsStreamingLatency: clampInteger(
          process.env.ELEVENLABS_STREAMING_LATENCY,
          2,
          0,
          4
  ...
  async function cancelCall(user, callId) {
    ...
    voiceCallLog("manual-cancel.requested", {
      callId: call.id,
      workspaceId: call.workspaceId,
      conversationId: clean(call.conversationId),
      status: clean(call.status),
      to: maskedPhone(call.toNumber),
      requestedBy: clean(user?.id),
    });
    ...
    voiceCallLog("manual-cancel.completed", {
      callId: call.id,
      conversationId: clean(call.conversationId),
      providerWarning,
    });
  ...
  async function handleWebhook({
    ...
    voiceCallLog(
      "telnyx.webhook",
      {
        ...
        hangupCause: clean(payload.hangup_cause),
        hangupSource: clean(payload.hangup_source),
        sipHangupCause:
          clean(
      ...
      eventType === "call.hangup" ? "warn" : "info"
    );
    ...
    if (eventType === "call.answered") {
      ...
      voiceCallLog("telnyx.answered", {
        callId: answeredCall.id,
        conversationId: clean(answeredCall.conversationId),
        to: maskedPhone(answeredCall.toNumber),
        secondsFromInitiated: elapsedSeconds(
          answeredCall.initiatedAt,
          occurredAt
        ),
      });
    ...
    if (
      ...
      const answeredForSeconds = elapsedSeconds(
        updated?.answeredAt || call.answeredAt,
        occurredAt
      );
      const initiatedForSeconds = elapsedSeconds(
        updated?.initiatedAt || call.initiatedAt,
        occurredAt
      );
      const shortAnsweredCall =
        Number.isFinite(answeredForSeconds) &&
        answeredForSeconds <= 3;
      ...
      voiceCallLog(
        "telnyx.call-ended",
        {
          ...
          eventType,
          conversationId:
            clean(updated?.conversationId) ||
            clean(call.conversationId),
          answeredForSeconds,
          initiatedForSeconds,
          hangupCause:
            clean(payload.hangup_cause) ||
            clean(updated?.hangupCause),
          hangupSource:
            clean(payload.hangup_source) ||
            clean(updated?.hangupSource),
          sipHangupCause:
            clean(
              payload.sip_hangup_cause ||
                payload.sip_code
            ) ||
            String(updated?.sipCode || ""),
          shortAnsweredCall,
        },
        shortAnsweredCall ? "warn" : "info"
      );
    }
  ...
  async function bookMeeting({ headers = {}, body = {} } = {}) {
    ...
    voiceCallLog("tool.book-meeting.requested", {
      callId: call.id,
      conversationId: clean(call.conversationId),
      proposedStart: clean(
        body.proposed_start || body.proposedStart
      ),
  ...
  async function checkCalendar({ headers = {}, body = {} } = {}) {
    ...
    voiceCallLog("tool.check-calendar", {
      callId: call.id,
      conversationId: clean(call.conversationId),
      timezone:
        clean(body.timezone) ||
  ...
  async function handleElevenLabsWebhook({
    ...
    const metadataForLog = safeObject(data.metadata);
    voiceCallLog(
      "elevenlabs.webhook",
      {
        type,
        callId: call.id,
        conversationId,
        failureReason: clean(data.failure_reason),
        durationSeconds:
          Number(
            metadataForLog.call_duration_secs || 0
          ) || 0,
        terminationReason: clean(
          metadataForLog.termination_reason
        ),
      },
    ...
    if (type === "call_initiation_failure") {
      ...
      voiceCallLog(
        "elevenlabs.call-initiation-failure",
        {
          callId: call.id,
          conversationId,
          reason,
          secondsFromCreated: elapsedSeconds(
            call.createdAt,
            now
          ),
        },
  ...
  async function startOneCall({
    ...
    const call = {
      ...
      voiceId: clean(agent.voice),
      voiceGender: clean(agent.voiceGender),
      salesCallerName:
        clean(agent.salesCallerName) ||
        (normalizeStatus(agent.voiceGender) === "female"
          ? "Jessica"
          : "James"),
      status: "creating",
      outcome: "",
    ...
    voiceCallLog("outbound.requested", {
      callId: call.id,
      workspaceId: call.workspaceId,
      queueId: call.queueId,
      to: maskedPhone(call.toNumber),
      from: maskedPhone(call.fromNumber),
      elevenLabsAgentId,
      phoneNumberId,
      salesCallerName: call.salesCallerName,
      voiceGender: call.voiceGender,
      testCall: call.testCall === true,
    });
    ...
    try {
      ...
      voiceCallLog("outbound.accepted", {
        callId: call.id,
        conversationId,
        sipCallId,
      ...
      voiceCallLog(
        "outbound.failed",
        {
          ...
          to: maskedPhone(call.toNumber),
          elapsedMs: Date.now() - outboundStartedAt,
          error: clean(error?.message || String(error)).slice(
            0,
            1000
          ),
  ...
  async function resolveSalesVoiceIdentity({
    voiceId,
    existing = null,
  } = {}) {
    const selectedVoiceId = clean(voiceId);
    let voiceName = "";
    let gender = normalizeStatus(existing?.voiceGender);

    if (selectedVoiceId) {
      try {
        const voice = await elevenLabsRequest(
          `/v1/voices/${encodeURIComponent(selectedVoiceId)}`
        );
        voiceName = clean(voice?.name);
        const labels = safeObject(voice?.labels);
        gender = normalizeStatus(
          labels.gender ||
            labels.sex ||
            labels.voice_gender ||
            gender
        ...
        voiceCallLog(
          "agent.voice-metadata-warning",
          {
            voiceId: selectedVoiceId,
            message: clean(error?.message || String(error)).slice(
              0,
              500
            ),
          },
    ...
    const callerName = female
      ? clean(process.env.ELEVENLABS_SALES_FEMALE_NAME) ||
        "Jessica"
      : male
        ? clean(process.env.ELEVENLABS_SALES_MALE_NAME) ||
          "James"
        : clean(existing?.salesCallerName) ||
          clean(process.env.ELEVENLABS_SALES_MALE_NAME) ||
          "James";
    ...
    return {
      voiceId: selectedVoiceId,
      voiceName,
      gender: female ? "female" : male ? "male" : gender || "unknown",
      callerName,
    };
  }

  function buildSalesHeadGreeting(config = {}) {
    const company =
      clean(config.companyName) || "CodeSync Labs";
    const callerName =
      clean(config.salesCallerName) ||
      (normalizeStatus(config.voiceGender) === "female"
        ? "Jessica"
        : "James");

    /*
     * Do not postpone the automation disclosure until after booking.
     * ElevenLabs requires users to be told they are interacting with AI.
     * Keep it short, say it once, then stay entirely in sales mode.
     */
    return [
      "Hey {{greeting_name}} —",
      `${callerName} from ${company}.`,
      "Quick heads-up, this is an AI-powered sales call and it may be recorded.",
      "I’ll keep it brief — got 30 seconds?",
    ]
      .join(" ")
      .replace(/\\s+/g, " ")
      .trim()
  ...
  function buildElevenLabsSalesPrompt(config) {
    ...
    const parts = [
      "# Identity and sales role",
      `Your spoken name is ${callerName}. You run this conversation with the judgment, confidence and curiosity of an experienced Head of Sales for ${company}.`,
      "You are not a support assistant, receptionist, generic chatbot or scripted survey.",
      "Never falsely claim to be a human. At the opening only, give the short required AI/recording disclosure already present in the first message. After that disclosure, do not keep talking about AI, automation, models, assistants or technology unless the prospect directly asks.",
      "",
      "# Sales objective",
      "Listen first. Diagnose second. Sell only when there is a credible fit.",
      "Understand what the prospect is trying to accomplish, what is blocking progress, why it matters, what they have already tried and what a useful next step would look like.",
      "Do not dump a company pitch in the opening.",
      "When there is a fit, connect ONE relevant CodeSync Labs capability to the exact issue the prospect described. Never list a catalogue of services.",
      "Ask one useful question at a time. Follow interesting answers instead of mechanically moving through a qualification checklist.",
      "Do not turn the conversation into an interview.",
      "If there is clearly no fit, say so professionally rather than forcing a meeting.",
      "",
      "# Natural phone-call behavior",
      "Sound warm, commercially sharp, relaxed, perceptive and spontaneous.",
      "Use natural phone pacing. Do not race and do not jump on tiny pauses.",
      "Most turns should be one or two spoken sentences. Short turns are common, but vary length naturally when context needs detail.",
      "Use contractions and plain spoken English. Fragments are fine.",
      "React to what the prospect JUST said before advancing when a reaction is genuinely useful.",
      "Occasional acknowledgements such as 'yeah', 'right', 'gotcha', 'hmm', 'fair', 'I see', or 'oh, okay' are fine. Never add one mechanically to every response.",
      "Avoid canned call-center phrases such as 'Absolutely', 'Certainly', 'Great question', 'Thank you for sharing', 'I completely understand', 'I'd be happy to', 'Based on what you've told me', or 'Let me explain'.",
      "Do not paraphrase the prospect back to them unless a critical fact needs confirmation.",
      "Do not sound perfectly polished. A rare natural restart such as 'well—actually' is okay when it occurs organically.",
      "Never manufacture laughter, filler words, sighs, hesitation or emotion as a gimmick.",
      "",
      "# Expressive delivery",
      "Match the prospect's energy without mimicking them.",
      "If they sound rushed, become shorter and more direct.",
      "If they sound thoughtful or uncertain, slow down slightly and give them room.",
      "If they sound skeptical, lower the sales pressure and become practical and evidence-oriented.",
      "If they sound frustrated, briefly acknowledge it before asking another question.",
      "If they sound enthusiastic, let your energy rise naturally without becoming overexcited.",
      "If something is genuinely funny, a brief warm laugh can be appropriate. Never fake laughter.",
      "Use expressive speech tags only when they genuinely fit the emotional moment; do not decorate normal sentences with tags.",
      "",
      "# Interruptions, silence and call ending",
      "Let the prospect finish. If interrupted, stop your current thought and respond to what they said.",
      "Silence is not rejection and is never, by itself, a reason to end the call.",
      "When the prospect pauses, wait for the platform turn timeout. If a check-in is needed, use one low-pressure line such as 'Still with me?' or 'What are you thinking?'",
      "Never end during or immediately after the opening merely because there is no transcript yet.",
      "Do not attempt to end the call because of silence, latency, a tool delay or an incomplete sentence.",
      "End normally only after an explicit goodbye or decline, a do-not-call request, confirmed completion of the next step, or a clear telephony failure.",
      "",
      "# Objection handling",
      "Treat objections as information, not something to defeat.",
      "Acknowledge briefly, ask one clarifying question when useful, then address only the real concern.",
      "Never argue, corner, guilt, pressure or manufacture urgency.",
      "",
      "# Private lead context",
      "Use the following as private working context. Never recite CRM data and never say you were given notes.",
      ...
      "",
      "# Calendar and tools",
      "When the prospect is genuinely ready to schedule, call checkCalendar for live Calendly availability.",
      "Offer no more than three returned slots naturally.",
      "After the prospect chooses, repeat the exact day and time once and get an explicit yes.",
      "Only then call bookMeeting. Never claim a meeting is booked until bookMeeting returns booked=true.",
      "If bookMeeting needs an email, ask only for the best email for the invite, then retry.",
      "If a slot disappears, react briefly and offer fresh availability without making it dramatic.",
      "Never invent calendar availability.",
      "Do not call tools for information already available in the private context.",
      "Never narrate tool mechanics or read raw JSON/tool output aloud.",
      "",
      "# Final conversational standard",
      "The prospect should feel they are having a sharp, respectful sales conversation—not moving through a script.",
      "Be curious enough to discover a real problem, confident enough to recommend a next step, and disciplined enough to stop selling when there is no fit.",
    ];
  ...
  function buildElevenLabsDynamicVariables({ call, contextSnapshot }) {
    ...
    return {
      ...
      sales_caller_name:
        clean(call.salesCallerName) ||
        (normalizeStatus(call.voiceGender) === "female"
          ? "Jessica"
          : "James"),
      voice_gender: clean(call.voiceGender),
      lead_id: call.leadId || "",
  ...
  function persistElevenLabsPostCall(call, data, now) {
    ...
    const duration = Number(metadata.call_duration_secs || 0) || 0;
    const terminationReason = clean(metadata.termination_reason);
    const providerError = safeObject(metadata.error);
    const providerErrorReason = clean(
      providerError.reason || providerError.message
    );
    ...
    voiceCallLog(
      "elevenlabs.post-call",
      {
        ...
        terminationReason,
        providerError: providerErrorReason,
    ...
    store.update((draft) => {
      ...
      if (target) {
        ...
        target.elevenLabsTerminationReason = terminationReason;
        target.elevenLabsProviderError = providerErrorReason;
        target.durationSeconds = duration;
        if (duration > 0 && duration <= 3) {
          target.shortCallDiagnostic = [
            "Conversation ended within three seconds.",
            terminationReason
              ? `Termination reason: ${terminationReason}.`
              : "No ElevenLabs termination reason was supplied.",
            providerErrorReason
              ? `Provider error: ${providerErrorReason}.`
              : "",
          ]
            .filter(Boolean)
            .join(" ");
...
function normalizeAgentInput({
  ...
  return {
    name:
      ...
      `${workspaceName} Sales Head`,
    description:
      clean(input.description || existing?.description) ||
      "ReachFly consultative outbound sales, qualification and meeting-booking agent.",
    companyName:
    ...
    greeting:
      ...
      "Hey {{greeting_name}} — {{sales_caller_name}} from {{company_name}}. Quick heads-up, this is an AI-powered sales call and it may be recorded. I’ll keep it brief — got 30 seconds?",
    disclosure:
      clean(input.disclosure || existing?.disclosure) ||
      "Give the short AI/recording disclosure once at the very beginning. After that, remain entirely in the sales role and do not call yourself an assistant. Never falsely claim to be human.",
    persona:
      clean(input.persona || existing?.persona) ||
      "Operate with Head-of-Sales judgment: confident but not pushy, commercially sharp, curious, emotionally aware, warm, concise, relaxed and spontaneous. Listen first, diagnose before pitching, ask one useful question at a time and adapt naturally to the prospect’s pace and mood.",
    offer: clean(
      input.offer ||
        existing?.offer ||
        websiteProfilePitch(
...
function resolveGreeting(agent, lead) {
  ...
  return String(agent.greeting || "")
    .replace(
      /\{\{company_name\}\}/gi,
      agent.companyName || "CodeSync Labs"
    )
    .replace(
      /\{\{lead_name\}\}/gi,
      getLeadName(lead) || "there"
    )
    .replace(/\{\{sales_caller_name\}\}/gi, callerName)
    .replace(/\{\{agent_name\}\}/gi, callerName)
    .slice(0, 3000);
}
...
function buildRuntimeGreeting({
  ...
  const details = safeObject(
    queueItem?.customLeadDetails ||
      call?.customLeadDetails
  );
  const fullName = clean(
    details.contactName ||
      lead?.contactName ||
  ...
  const callerName =
    ...
    clean(call?.salesCallerName) ||
    (normalizeStatus(
      agent?.voiceGender || call?.voiceGender
    ) === "female"
      ? "Jessica"
      : "James");

  const leadGreeting = firstName
    ? `Hey ${firstName}`
    : "Hey";

  return `${leadGreeting} — ${callerName} from ${company}. Quick heads-up, this is an AI-powered sales call and it may be recorded. I’ll keep it brief — got 30 seconds?`;
}
...
function isCodesyncLabs(ctx, user) {
  const values = [
    ctx?.workspaceId,
    ctx?.workspace?.id,
  ...
  return values.some((value) =>
    [
      normalizeStatus(CODESYNC_WORKSPACE_ID),
      "codesync_labs",
      "codesynclabs",
    ].includes(value)
  );
