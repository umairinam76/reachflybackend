...
export function createStore({ dataDir = "./data" } = {}) {
  ...
  function update(mutator) {
    ...
    const unwrappedResult =
      unwrapTrackedProxy(result);

    if (
      unwrappedResult &&
      unwrappedResult !== currentState &&
      unwrappedResult !== unwrapTrackedProxy(tracked)
    ) {
      currentState = mergeState(
        unwrappedResult
      );
...
/*
 * Keep the proxy -> raw-target registry outside createTrackedProxy().
 *
 * This is intentionally module-scoped. A tracked proxy created during one
 * store.update() must still be recognizable if a service accidentally tries
 * to assign it during a later update. Keeping the WeakMap inside one update
 * would only protect that single mutation cycle and can allow Proxy-on-Proxy
 * layers to accumulate over time.
 */
const TRACKED_PROXY_TARGETS = new WeakMap();

function unwrapTrackedProxy(value) {
  let current = value;
  let depth = 0;
  ...
  while (
    ...
    TRACKED_PROXY_TARGETS.has(current) &&
    depth < 100
  ) {
    current = TRACKED_PROXY_TARGETS.get(current);
    depth += 1;
  }
...
function unwrapDescriptor(descriptor) {
  ...
  return {
    ...descriptor,
    value: unwrapTrackedProxy(descriptor.value),
  };
}
...
function createTrackedProxy(root, touchedRoots) {
  ...
  function wrap(input, rootKey = "") {
    const target = unwrapTrackedProxy(input);
    ...
    const proxy = new Proxy(target, {
      ...
      set(object, property, value) {
        ...
        if (key) {
          touchedRoots.add(key);
        }

        /*
         * Do not pass the Proxy as Reflect.set's receiver. For ReachFly's
         * plain-object/array state this is both sufficient and safer: using
         * the Proxy receiver can re-enter defineProperty/set traps and was a
         * contributor to the historical "Maximum call stack size exceeded"
         * failure when proxied values had leaked back into raw state.
         */
        return Reflect.set(
          object,
          property,
          unwrapTrackedProxy(value)
        );
      },
    ...
    TRACKED_PROXY_TARGETS.set(proxy, target);
    return proxy;
  }
