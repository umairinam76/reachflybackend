...
export function createResourceBoardService({
  ...
  function listWorkspaceMembers(user, state, workspaceId) {
    const byId = new Map();
    const byEmail = new Map();

    // Start with raw users so resource-only fields (dialer number, lead limit,
    // sender IDs) survive even when workspaceService returns a sanitized view.
    for (const member of state.users || []) {
      if (member.workspaceId !== workspaceId) continue;
      const copy = { ...member };
      if (copy.id) byId.set(clean(copy.id), copy);
      const emailKey = normalizeEmail(copy.email);
      if (emailKey) byEmail.set(emailKey, copy);
    }

    try {
      const listed = workspaceService?.listMembers?.(user);
      for (const member of Array.isArray(listed) ? listed : []) {
        const memberId = clean(member.id || member.userId || member.memberId);
        const emailKey = normalizeEmail(member.email);
        const existing =
          (memberId && byId.get(memberId)) ||
          (emailKey && byEmail.get(emailKey)) ||
          null;
        const merged = mergeWorkspaceMember(existing, member);

        if (memberId) {
          merged.id = memberId;
          byId.set(memberId, merged);
        }
        if (emailKey) {
      ...
      // Raw workspace users remain the fallback/source of truth.
    }

    const values = [...byId.values()];
    for (const member of byEmail.values()) {
      if (!member.id || !byId.has(clean(member.id))) {
        values.push(member);
      }
    }
    ...
    return values
      .filter(Boolean)
      .sort((left, right) => {
        const leftRole = normalizeRole(left.workspaceRole || left.role);
        const rightRole = normalizeRole(right.workspaceRole || right.role);
        const order = { owner: 0, admin: 1, manager: 2, caller: 3 };
        const roleDiff = (order[leftRole] ?? 9) - (order[rightRole] ?? 9);
  ...
  function getBoard(user, query = {}) {
    ...
    const workspaceMembers = listWorkspaceMembers(user, state, ctx.workspaceId);
    const callers = workspaceMembers.filter(
      (member) =>
        normalizeRole(member.workspaceRole || member.role) === "caller" &&
        member.active !== false &&
        member.isActive !== false
    );
    const callerMap = new Map(
      callers.map((member) => [clean(member.id), member])
    );
    const assignments = collectAssignments(state, ctx.workspaceId).map(
      (assignment) => enrichAssignmentIdentity(assignment, callerMap)
    );
    const tasks = safeListTasks(user, query).map(
      (task) => enrichTaskIdentity(task, callerMap)
    );
    ...
    const resources = callers.map((member) =>
      buildResource({
        ...
        leadStats:
          boardStats.leadsByResource.get(member.id) || null,
        taskStats:
          boardStats.tasksByResource.get(member.id) || null,
    ...
    return {
      ...
      summary: {
        ...
        teamMembers: workspaceMembers.filter(
          (member) => member.active !== false && member.isActive !== false
        ).length,
        owners: workspaceMembers.filter(
          (member) => normalizeRole(member.workspaceRole || member.role) === "owner"
        ).length,
        admins: workspaceMembers.filter(
          (member) => normalizeRole(member.workspaceRole || member.role) === "admin"
        ).length,
        managers: workspaceMembers.filter(
          (member) => normalizeRole(member.workspaceRole || member.role) === "manager"
        ).length,
        callers: callers.length,
        leads: assignments.length,
        unassignedLeads:
          boardStats.unassignedLeads,
        activeLeads:
          boardStats.activeLeads,
        tasks: tasks.length,
        openTasks:
          boardStats.openTasks,
  ...
  function listUnassignedLeads(user, query = {}) {
    const state = store.read();
    const ctx = requireManagerAccess(user, state);
    ...
    for (const campaign of state.campaigns || []) {
      if (campaign.workspaceId !== ctx.workspaceId) {
        continue;
      }

      for (const lead of campaign.leads || []) {
        const assigneeId = clean(
          lead.assignedTo ||
            lead.assigneeId ||
            lead.assignedUserId
        );
    ...
    const leads = candidates
      ...
      .map(({ campaign, lead }) => {
        const assignment =
          publicAssignment(campaign, lead);
  ...
  function assignLead(user, assignmentId, input = {}) {
    ...
    store.update((draft) => {
      ...
      // Keep all historical assignment aliases synchronized. Different ReachFly
      // screens were introduced at different times and some still read one of
      // these legacy fields. Writing them together prevents a lead from looking
      // assigned on one screen and unassigned/Unknown on another.
      lead.assignedTo = targetId;
      lead.assigneeId = targetId;
      lead.assignedUserId = targetId;
      lead.assignedToUserId = targetId;
      lead.assignedToName = targetName;
      lead.assigneeName = targetName;
  ...
  function setResourceLimit(user, resourceId, input = {}) {
    ...
    store.update((draft) => {
      ...
      if (
        ...
        typeof draft.resourceLeadLimits[ctx.workspaceId] !== "object" ||
        Array.isArray(draft.resourceLeadLimits[ctx.workspaceId])
      ) {
        draft.resourceLeadLimits[ctx.workspaceId] = {};
      }
...
function mergeWorkspaceMember(existing, incoming) {
  if (!existing) {
    return { ...incoming };
  }

  // Prefer non-empty values from the authoritative workspace-member view, but
  // preserve resource/dialer fields that only exist on the raw user record.
...
function enrichAssignmentIdentity(assignment, callerMap) {
  const assigneeId = getAssignmentUserId(assignment);
  const member = assigneeId ? callerMap.get(assigneeId) : null;
  const name =
...
function enrichTaskIdentity(task, callerMap) {
...
function buildResource({
  ...
  const dailyLeadLimit = getResourceLimit(
    state,
    workspaceId,
    member
  );
  ...
  const completedLeadCount = Number(
    leadStats?.completed || 0
  );
  const totalLeadCount = Number(
    leadStats?.total || 0
  );
  ...
  return {
    ...
    completedLeadCount,
    totalLeadCount,
    remainingLeadCapacity: Math.max(
      0,
      dailyLeadLimit - activeLeadCount
    ),
    leadUtilizationPercent: Math.min(
      100,
      Math.round(
        (activeLeadCount /
          Math.max(1, dailyLeadLimit)) *
          100
      )
    ),
    openTaskCount,
...
function buildBoardStats(
  ...
  for (const assignment of assignments || []) {
    ...
    const current =
      leadsByResource.get(assigneeId) || {
        total: 0,
        active: 0,
        completed: 0,
      };
  ...
  for (const task of tasks || []) {
    ...
    const current =
      tasksByResource.get(assigneeId) || {
        total: 0,
        open: 0,
        completed: 0,
      };
...
function publicAssignment(campaign, lead) {
  ...
  return {
    ...
    assigneeId,
    assignedTo: assigneeId,
    assignedToName: lead.assignedToName || lead.assigneeName || "",
    assigneeName: lead.assigneeName || lead.assignedToName || "",
...
function publicTask(task) {
  ...
  return {
    ...
    assigneeId: clean(
      task.assigneeId || task.assignedToUserId || task.assignedUserId || task.assignedTo
    ),
    assignedToUserId: clean(
      task.assignedToUserId || task.assigneeId || task.assignedUserId || task.assignedTo
    ),
    assigneeName: task.assigneeName || task.assignedToName || "",
    assignedToName: task.assignedToName || task.assigneeName || "",
...
function appendLeadTimeline(lead, event) {
  ...
  if (lead.timeline.length > 500) {
    lead.timeline.splice(500);
  }
}
...
function ensureBoardState(draft) {
  if (
    ...
    typeof draft.resourceLeadLimits !== "object" ||
    Array.isArray(draft.resourceLeadLimits)
  ) {
    draft.resourceLeadLimits = {};
  }

  if (!Array.isArray(draft.resourceEmailAssignments)) {
    draft.resourceEmailAssignments = [];
  }

  if (!Array.isArray(draft.resourceBoardActivity)) {
    draft.resourceBoardActivity = [];
  }

  if (!Array.isArray(draft.telnyxDialers)) {
    draft.telnyxDialers = [];
  }
}
...
function appendActivity(draft, event) {
  ...
  if (draft.resourceBoardActivity.length > 500) {
    draft.resourceBoardActivity.splice(500);
  }
}
