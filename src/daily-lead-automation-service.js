...
const DEFAULT_MINI_PREFETCH_ENABLED =
  envFlag("DAILY_DEFAULT_MINI_PREFETCH_ENABLED", false);
...
export function createDailyLeadAutomationService({
  ...
  function startScheduler() {
    ...
    // Realtime audit mode: no startup/background Mini prefetch.
    // A Mini Audit is created only when the caller opens an individual lead.
    console.log(
      "[daily-leads] default Mini prefetch disabled; audits generate on caller open."
    );
  ...
  async function executeWorkspaceRun({
    ...
    try {
      ...
      // Production realtime rule: daily allocation NEVER creates audit jobs.
      // The default Mini Audit is generated only when the caller opens/clicks
      // an individual lead. This keeps allocation lightweight and prevents
      // hundreds of dormant audit jobs from loading the small API host.
      const auditQueuedCount = 0;
