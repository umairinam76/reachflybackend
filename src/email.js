...
export function createEmailService({ store }) {
  ...
  async function getPooledTransport(settings = {}) {
    ...
    const transport = nodemailer.default.createTransport({
      ...
      maxConnections: clampNumber(
        process.env.SMTP_MAX_CONNECTIONS || (isGmail ? 1 : 4),
        1,
        10
      ),
      maxMessages: clampNumber(process.env.SMTP_MAX_MESSAGES || 100, 10, 1000),

      rateDelta: clampNumber(process.env.SMTP_RATE_DELTA || 1000, 100, 60000),
      rateLimit: clampNumber(
        process.env.SMTP_RATE_LIMIT || (isGmail ? 1 : 6),
        1,
        100
      ),
  ...
  async function syncInbox(userId, options = {}) {
    ...
    const emailSettings = getUserEmailSettings(state, ownerId);
    const accounts = getAccountsFromSettings(emailSettings);
    const settings = getAccountByIdOrActive(
      emailSettings,
      options.accountId,
      accounts
    );
